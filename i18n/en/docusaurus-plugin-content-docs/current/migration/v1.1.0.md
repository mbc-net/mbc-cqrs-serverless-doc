...
### Migration Steps for Deprecated Methods
...
1. **`publish()` / `publishPartialUpdate()`**: The Async suffix clarifies that these methods use DynamoDB Streams for asynchronous data synchronization

2. **`genNewSequence()`**: This method was replaced by generateSequenceItem which provides a more consistent API and better return type

## New Features in v1.1.0

### Internal Normalization Behavior

v1.1.0 applies tenant code normalization internally. Tenant codes are lowercased automatically by the framework — no utility import is required. The `common` tenant is stored as `'common'` (lowercase) regardless of input case.
