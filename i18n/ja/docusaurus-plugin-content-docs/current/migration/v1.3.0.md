...
- `AppSyncEventsService` — AWS AppSync Events HTTP APIを使用したオプトインのリアルタイム通知トランスポート（有効化するには`NOTIFICATION_TRANSPORTS=appsync-event`を設定）。セットアップの詳細は[通知モジュール](/docs/notification-module#appsync-events-service)を参照。
...
- [変更履歴 v1.3.0](/docs/changelog#v130) - v1.3.0 リリースノート
- [メールサービス](/docs/email-service) - nodemailerの設定とTLSセキュリティ
- [通知モジュール](/docs/notification-module#appsync-events-service) - AppSync Eventsのセットアップと設定
