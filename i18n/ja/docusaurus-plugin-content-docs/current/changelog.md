...
- **core:** SES v2インラインテンプレートメールのサポートを追加 ([詳細を見る](./email-service#inline-template-emails))
  - テンプレートメール送信用の新しい`sendInlineTemplateEmail()`メソッド
  - 変数付きメールテンプレート定義用の`InlineTemplateContent`インターフェース
  - テンプレートベースのメール通知用`TemplatedEmailNotification`インターフェース
  - 自動テンプレート変数置換によるローカル開発サポート
  - IAMポリシーを更新して`ses:SendTemplatedEmail`権限を追加
