...
- **mcp-server:** AP028、AP029、AP030 のアンチパターン検出器を追加 ([詳細を見る](/docs/mcp-server#anti-pattern-detection)) ([PR #460](https://github.com/mbc-net/mbc-cqrs-serverless/pull/460), [PR #461](https://github.com/mbc-net/mbc-cqrs-serverless/pull/461))
  - AP028: NestJS モジュール間での `DataSyncHandler` 重複登録 — 検出して重複排除し、警告ログを出力
  - AP029: `@DataSyncHandler` の予約済み `'dynamodb'` タイプを使用するとハンドラーがディスパッチから静かに除外される
  - AP030: `@DataSyncHandler` で完全修飾テーブル名（`'-command'` で終わる）を使用するとハンドラーが静かにスキップされる

### バグ修正

- **core:** `DataSyncHandler` の重複登録を修正 — `ExplorerService` が `Set` でハンドラーを重複排除し、`CommandService` が `Map<constructor.name, instance>` で重複排除して削除された重複ごとに `warn` ログを出力するように ([PR #459](https://github.com/mbc-net/mbc-cqrs-serverless/pull/459))
- **core:** `publishSync` を修正 — ハンドラーディスパッチ前にバージョン付き `sk` が `command.sk` に代入されるようになり `IDataSyncHandler.up()` が正しいキーを受け取る。`updateStatus` エラーがサイレントにマスクされなくなった。デバッグを容易にするため `checkNextToken` の警告ログに `pk`/`sk` を追加。 ([詳細を見る](/docs/command-service#publishsync-audit-trail)) ([PR #460](https://github.com/mbc-net/mbc-cqrs-serverless/pull/460))

### セキュリティ

- npm 依存関係を強化 — `overrides`（`lodash`、`tmp`、`multer`、`ajv`、`picomatch`、`fast-xml-parser`）と個別アップグレード（`@nestjs/config` ^4.0.4、`@typescript-eslint` v8）で high/moderate 脆弱性を解消。本番ルート監査: critical/high 0件 ([PR #445](https://github.com/mbc-net/mbc-cqrs-serverless/pull/445)–[#453](https://github.com/mbc-net/mbc-cqrs-serverless/pull/453))
- **cli:** スキャフォールドテンプレートの `multer` を v2.2.0 にアップグレードして CVE-2026-5079（深いネストのマルチパートフィールド名による DoS）を修正 ([PR #463](https://github.com/mbc-net/mbc-cqrs-serverless/pull/463))
- ブロッキング CI ゲートを追加: `npm audit --omit=dev --audit-level=high` と ESLint 起動チェックがすべての PR で実行され、本番脆弱性があるとビルドが失敗する ([PR #448](https://github.com/mbc-net/mbc-cqrs-serverless/pull/448))
...
- **core:** グループベースのロール認可を追加 — `RolesGuard` はまず `custom:roles` の直接ロールを確認し、次に新しい `custom:groups` クレームに含まれるユーザーのグループから導出されたロールを確認します ([詳細を見る](/docs/authentication#group-based-roles)) ([PR #440](https://github.com/mbc-net/mbc-cqrs-serverless/pull/440))
  - 新しい `@GroupRoleResolver()` デコレータと `IGroupRoleResolver` インターフェース — アプリごとに 1 つだけリゾルバを実装し、グループ ID をロールにマッピングします（DynamoDB/RDS/設定から読み込み）。マッピングは JWT には保存されません
  - `UserContext` に `tenantRoles`（直接ロールの配列）と `tenantGroupIds`（アクティブテナントのグループ ID）が追加されました。`tenantRole`（単数）は後方互換性のため維持されます
  - オプトイン: 既存の `@Roles()` チェックはコード変更なしで動作し続けます
  - リゾルバの失敗は（暗黙の 403 ではなく）5xx として伝播するため、バックエンド障害を本来のアクセス拒否と区別できます
- **mcp-server:** v1.3.1 向けにスキルとアナライザを更新
  - AP027 アンチパターン検出を追加: `@GroupRoleResolver()` クラスに `@Injectable()` を併用してはいけません
  - `mbc-migrate` スキル: v1.3.1 のマイグレーションガイドを追加
  - `mbc-generate` スキル: グループロールリゾルバのコード生成テンプレートを追加
  - `mbc-review` スキル: AP022（グループロールリゾルバ）をチェックリストに追加

### バグ修正

- **core:** 不正な `custom:groups` クレームを例外ではなく許容（グループロールなしにフェイルクローズ）するようにし、不正なトークンでもリクエストをクラッシュさせず直接ロールのみのチェックに縮退します
- [移行ガイド v1.3.1](/docs/migration/v1.3.1) でアップグレード手順を確認してください。
...
- **core:** `AppSyncEventsService` を追加 — AWS AppSync Events API によるリアルタイム通知のオプトイン HTTP pub/sub トランスポート ([詳細を見る](/docs/notification-module#appsync-events-service)) ([PR #433](https://github.com/mbc-net/mbc-cqrs-serverless/pull/433))
  - `NOTIFICATION_TRANSPORTS` 環境変数（カンマ区切り）でトランスポートを選択。デフォルトは `appsync-graphql` — 既存の GraphQL サブスクリプションの動作は変わりません
  - チャンネル構造: `/{namespace}/{tenantCode}/{action}/{sanitizedId}` — クライアントは AppSync ワイルドカード（`/*`）で任意の粒度でサブスクライブ可能
  - 認証: Lambda/ECS パブリッシュには IAM SigV4、クライアントサブスクライブには API キー
  - デュアルパブリッシュモード: 移行中は `NOTIFICATION_TRANSPORTS=appsync-graphql,appsync-event` で両方のトランスポートに同時パブリッシュ
  - 新規環境変数: `NOTIFICATION_TRANSPORTS`, `APPSYNC_EVENTS_ENDPOINT`, `APPSYNC_EVENTS_NAMESPACE` ([詳細を見る](/docs/environment-variables#appsync-events-env))
  - `@NotificationTransport` デコレータ: プラグイン式トランスポート登録 — フレームワークコードを変更せずにカスタムトランスポートを追加可能
- **cli:** CDK インフラテンプレート: `Config` 型に `appsyncEvents` と `notificationTransports` を追加 — `EventApi` + `ChannelNamespace` をプロビジョニングし、Lambda/ECS に `NOTIFICATION_TRANSPORTS`、`APPSYNC_EVENTS_ENDPOINT`、`APPSYNC_EVENTS_NAMESPACE` を注入、`grantPublish()` で `appsync:EventPublish` IAM 権限を自動付与
- **mcp-server:** v1.3.0 向けスキルとアナライザーを更新 ([PR #436](https://github.com/mbc-net/mbc-cqrs-serverless/pull/436))
  - AP026 アンチパターン検出器を追加: `INotificationTransport` 実装クラスに `@NotificationTransport` と `@Injectable` を二重指定するケース
  - `mbc-migrate` スキル: v1.3.0 移行ガイドおよびバージョン互換性テーブルを更新
  - `mbc-generate` スキル: カスタム通知トランスポートのコード生成テンプレートを追加
  - `mbc-review` スキル: チェックリストに AP026 を追加
- [移行ガイド v1.3.0](/docs/migration/v1.3.0) でアップグレード手順を確認してください。
...
- **core:** Repository の Read-Your-Writes（RYW）改善 — データテーブルがセッションバージョンに追いついた／追い越した時点で陳腐化したRYWセッションを即時パージし、「stale override」読み取りを解消 ([詳細を見る](/docs/command-service#ryw-session-cleanup)) ([PR #410](https://github.com/mbc-net/mbc-cqrs-serverless/pull/410))
  - `getItem`: `existing.version >= session.version` の場合、バックグラウンドでセッションをパージし永続化済みデータを直接返却（不要な Command テーブル読み取りをスキップ）
  ...
  - `SessionService.delete()` と `DynamoDbService.deleteItem()` を追加（fire-and-forgetでの利用に安全）
  - 後方互換: `getVersion` はオプション。`RYW_SESSION_TTL_MINUTES` 未設定のプロジェクトは影響なし
- **mcp-server:** AP021 アンチパターン検出器を追加 — `CommandService 内で publishAsync 直後にイベントをemit`（Severity: High） ([詳細を見る](/docs/mcp-server#code-analysis-tools)) ([PR #408](https://github.com/mbc-net/mbc-cqrs-serverless/pull/408))
...
- **mcp-server:** 検出器AP番号（`analyze.ts`）とskill-doc AP番号（`skills/mbc-review/SKILL.md`）のクロスリファレンス表を追加 — 両者は独立した番号体系で、コードが一致するのは AP016〜AP019 と AP021 のみ ([PR #411](https://github.com/mbc-net/mbc-cqrs-serverless/pull/411))
  - 検出器の出力に対応するskill-doc AP番号を併記するように変更（例: `AP005: Hardcoded Tenant (skill-doc: AP002)`）
  - `mbc-migrate` スキル: v1.2.5・v1.2.6セクションとマイグレーションマトリクスのエントリを追加
  - `mbc-debug` スキル: v1.2.6での（意図的な）RYWセッション早期パージに関するトラブルシューティング項目を追加

### バグ修正

- **mcp-server:** `server.ts` がパッケージバージョンをハードコードしており、`lerna version` のたびにサイレントにずれていた問題を修正。実行時に `package.json` から読み込むように変更 ([PR #411](https://github.com/mbc-net/mbc-cqrs-serverless/pull/411))
...
- **import:** ZIPインポート処理をリファクタリング — ZIPジョブ処理を`ImportService`に直接統合し、独立した`ZipImportQueueEventHandler`を削除 ([詳細を見る](/docs/import#zip-import-architecture)) ([PR #404](https://github.com/mbc-net/mbc-cqrs-serverless/pull/404))
  - `ZipImportQueueEventHandler`を削除。ZIPインポートジョブは`ImportService`内で直接処理されるように変更
  - `ImportEventHandler`が`ZIP_MASTER_JOB`イベントに対してSQSパブリッシュをスキップするように変更
  - `CreateZipImportDto`のZIPインポートバリデーションを強化
  - ZIP処理のエラーハンドリングとロギングを改善
- **mcp-server:** `mbc_check_anti_patterns`ツールにAP016〜AP020のアンチパターン検出器を追加（合計20パターン） ([PR #403](https://github.com/mbc-net/mbc-cqrs-serverless/pull/403))
  - AP016: リスロー前のエラーロギング欠落（High）
  - AP017: 部分更新時の属性マージ誤り（High）
  - AP018: Swaggerドキュメント / `@ApiTags` 欠落（Low）
  - AP019: リストクエリのページネーション欠落（High）
  - AP020: トレーシング用`getCommandSource`欠落（Low）
- **mcp-server:** `mbc-migrate`と`mbc-debug`スキルをv1.1.x・v1.2.x破壊的変更ガイドで更新 ([PR #403](https://github.com/mbc-net/mbc-cqrs-serverless/pull/403))

### 依存関係

- **mcp-server:** `@modelcontextprotocol/sdk`を1.26.0から1.29.0に更新 ([PR #403](https://github.com/mbc-net/mbc-cqrs-serverless/pull/403))
...
- **task:** `TaskModule.register()`がグローバルモジュールを返すようになり、複数のフィーチャーモジュールへの登録が不要になりました ([PR #398](https://github.com/mbc-net/mbc-cqrs-serverless/pull/398))
  - 以前はタスク機能が必要な各モジュールで`TaskModule.register()`を呼び出す必要がありましたが、`AppModule`で一度だけ登録すればよくなりました
- **mcp-server:** AP015アンチパターン検出器を追加 — v1.2.4以降で不要となった`TaskModule.register()`の重複呼び出しを検出します ([PR #399](https://github.com/mbc-net/mbc-cqrs-serverless/pull/399))
...
- **import:** SQS統合と親ジョブカウンタートラッキングでインポート処理を強化 ([PR #387](https://github.com/mbc-net/mbc-cqrs-serverless/pull/387))
  - `ImportQueueEventHandler`が分散処理のためにサブジョブイベントをSQSにパブリッシュするようになりました
  - 子ジョブがエンキューされたときに親ジョブカウンターが正しくインクリメントされるようになりました
- **import:** `CsvImportSfnEventHandler`がSUCCEEDEDとFAILEDの両方のStep Functions結果を処理するよう修正 ([PR #387](https://github.com/mbc-net/mbc-cqrs-serverless/pull/387))

### バグ修正

- **core:** `extractInvokeContext`で不正なJWTトークンを適切に処理するよう修正 ([PR #388](https://github.com/mbc-net/mbc-cqrs-serverless/pull/388))
  - 破損または予期しないJWTペイロードによる認証失敗を防止
...
- **core:** `publishSync()`と`publishPartialUpdateSync()`がコマンドに変更がない場合（no-op）に`null`を返すように変更 ([詳細を見る](/docs/command-service#publishsync-audit-trail)) ([PR #375](https://github.com/mbc-net/mbc-cqrs-serverless/pull/375))
...
- 両方の破壊的変更の詳細なアップグレード手順については、[マイグレーションガイド v1.2.0](/docs/migration/v1.2.0)を参照してください。
...
- **import:** ZIPオーケストレーターのインポートステータス処理を修正 ([PR #371](https://github.com/mbc-net/mbc-cqrs-serverless/pull/371)) (v1.2.0からのバックポート)
  - `ImportStatusHandler`がジョブ失敗時に`importJobStatus`を含むタスク成功を送信するように修正
  - `ZipImportSfnEventHandler`がCSVタスクの失敗数を集計し最終ジョブステータスを調整
...
- **core:** 高度な変数置換によるインラインテンプレートメールの機能強化 ([詳細を見る](/docs/notification-module#advanced-template-features))
  - テンプレート変数のネストプロパティアクセスをサポート（例：二重波括弧プレースホルダーでの user.profile.name）
  - テンプレート変数でのUnicode/日本語キーのサポート
  - プレースホルダー内の空白トリミング（変数名前後のスペースを無視）
...
- **mcp-server:** AI支援開発のためのコード分析ツールを追加 ([詳細を見る](/docs/mcp-server#code-analysis-tools))
...
- **import:** Step Functions CSVハンドラーが子ジョブの失敗に関わらず常にCOMPLETEDステータスを設定する問題を修正
  ...
  - 詳細は[CsvImportSfnEventHandler](/docs/import-export-patterns#csvimportsfneventhandler)を参照
...
- **import:** 子インポートジョブが失敗した際にマスタージョブのステータスがFAILEDに更新されない問題を修正
  ...
  - 詳細は[ImportQueueEventHandlerエラーハンドリング](/docs/import-export-patterns#import-error-handling)を参照
...
- **import:** Step Functionsの適切なエラーハンドリングのため、`ImportStatusHandler`に`SendTaskFailure`サポートを追加
  ...
  - 詳細は[ImportStatusHandler API](/docs/import-export-patterns#importstatushandler-api)を参照
...
- **master:** `MasterDataService.search()`の`masterTypeCode`比較を修正 - `settingCode`検索パラメータを部分一致（`contains`）から完全一致に変更 ([詳細を見る](/docs/master#search-parameters))
...
- [マイグレーションガイド v1.3.0](/docs/migration/v1.3.0) - v1.3.0アップグレードガイド（nodemailer v8）
- [マイグレーションガイド v1.2.0](/docs/migration/v1.2.0) - v1.2.0アップグレードガイド（publishSyncのnull返却）
- [マイグレーションガイド v1.1.0](/docs/migration/v1.1.0) - v1.1.0アップグレードガイド（TENANT_COMMON、テナントコードの正規化）
- [バージョン競合ガイド](/docs/version-conflict-guide) - アップグレード時のバージョン競合処理
- [Web変更履歴](/docs/web-changelog) - master-webおよびsurvey-webフロントエンドパッケージのリリースノート
