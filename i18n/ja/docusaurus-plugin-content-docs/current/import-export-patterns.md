...
| `countCsvRows(input)` | S3からCSVファイルの合計データ行数をカウント（ヘッダー行を除く） |

#### V2バッチ処理アーキテクチャ (v1.1.5+) {#v2-batch-processing}

:::info バージョンノート
v2バッチ処理アーキテクチャは[バージョン1.1.5](/docs/changelog#v115)で導入され、大規模インポート（例: 30万行以上）のスループットを大幅に向上させます。
:::

v1.1.5では、Hot Partitionボトルネックを解消するためCSVインポートパイプラインを再設計しました：

| 項目 | v1 (≤ 1.1.4) | v2 (1.1.5+) |
|------------|------------------|-----------------|
| バッチサイズ | `MaxInputBytesPerBatch: 10` | `MaxItemsPerBatch: 100` |
| 行処理 | 行ごとに`import_tmp`書き込み → SQS → Step Functions | LambdaバッチD内で`CommandService`に直接パブリッシュ |
| ジョブ完了処理 | `CommandFinishedHandler`経由で行ごとにアトミックカウンター更新 | `finalize_parent_job`ステートで`UpdateItem`を1回のみ実行 |
| 進捗追跡 | 行単位のリアルタイム | 完了時に一括集計 |

##### V2の破壊的変更 {#v2-batch-processing-breaking-changes}

:::danger 破壊的変更 (v1.1.5)
v1.1.5へのアップグレード時に以下のインフラ更新が必要です：

1. **リアルタイム進捗追跡なし**: `processedRows`・`succeededRows`・`failedRows`カウンターはStep Functions実行完了時に一度だけ更新されます。ジョブは`PROCESSING`のまま`COMPLETED`または`FAILED`に直接遷移します。
2. **`import_tmp`テーブルをバイパス**: 個別のCSV行は`import_tmp` DynamoDBテーブルに書き込まれなくなりました。
3. **ステートマシンの更新が必要**: `import-csv`ステートマシンに`finalize_parent_job`ステートと`resultPath: '$.processingResults'`が必要です。CDK（`infra-stack.ts`）と`serverless.yml`をこのパッケージと同時に更新してください。
:::

##### パブリッシュモードの設定 {#import-publish-mode}

v1.1.5では、エンティティごとのコマンドパブリッシュ方法を制御する`ImportPublishMode`を導入しました：

```typescript
// Register import module with per-entity publish mode (エンティティごとのパブリッシュモードでインポートモジュールを登録)
ImportModule.register({
  profiles: [
    {
      tableName: 'building',
      ...
      publishMode: ImportPublishMode.ASYNC, // Default: non-blocking (recommended for large imports) (デフォルト: ノンブロッキング（大規模インポートに推奨）)
    },
    {
      tableName: 'room',
      importStrategy: RoomImportStrategy,
      processStrategy: RoomProcessStrategy,
      publishMode: ImportPublishMode.SYNC,  // Blocking: use only for small batches (ブロッキング: 小規模バッチのみで使用)
    },
  ],
}),
```
...
:::warning
`ImportPublishMode.SYNC`はLambdaバッチ内で行ごとに`publishSync`を逐次実行します。`MaxItemsPerBatch: 100`の設定では、処理が遅い場合にLambdaの15分タイムアウトを超える可能性があります。大規模インポートには`ASYNC`を推奨します。
:::

#### 行数カウント (v1.1.3+) {#csv-total-row-counting}

v1.1.3・v1.1.4では`finalizeParentJob`が`countCsvRows()`でS3からCSVを読み込んで行数をカウントしていました。v1.1.5以降では、Step Functionsがネイティブに集約したバッチサマリーから行数を取得するため、S3の再読み込みは不要です。

:::info バージョンノート
v1.1.3で、AWS Step Functionsの256KBステートデータ制限を回避するため、行数カウントを`MapResult.length`から`countCsvRows()`に変更しました。Distributed Mapの`resultPath`は`DISCARD`に設定され、子実行結果の集約が防止されます。[バージョン1.1.3](/docs/changelog#v113)を参照してください。
:::
...
const status = failedRows > 0
  ? ImportJobStatus.FAILED    // Any child failed → FAILED (子ジョブが失敗した場合 → FAILED)
  : ImportJobStatus.COMPLETED // All children succeeded → COMPLETED (すべての子ジョブが成功した場合 → COMPLETED)
```
...
const status = failedRows > 0
  ? ImportJobStatus.COMPLETED  // Wrong! (間違い！)
  : ImportJobStatus.COMPLETED
```

このバグにより、子インポートジョブが失敗しても、Step FunctionsがSUCCESSを報告していました。詳細は[バージョン1.0.20](/docs/changelog#v1020)を参照してください。
:::
...
- [ディレクトリ](/docs/directory) - 署名付きURLによるS3ファイル・フォルダ管理
