---
description: SequencesModuleを使用して、カスタマイズ可能なフォーマットとローテーションサイクルでビジネスID用の自動インクリメントシーケンス番号を生成します。
---
...
- `rotateBy?: RotateByEnum`
  ...
  - ローテーション戦略の一覧:
    ...
    | 戦略 | カウンターがリセットされる条件 | 例 |
    |---|---|---|
    | `NONE` | なし — カウンターは無限にインクリメント | `1, 2, 3, … 9999` |
    | `DAILY` | 日付が変わったとき | 毎日深夜にリセット |
    | `MONTHLY` | 月が変わったとき | 毎月1日にリセット |
    | `YEARLY` | 年が変わったとき | 毎年1月1日にリセット |
    | `FISCAL_YEARLY` | 会計年度が変わったとき（開始月は `startMonth` で制御） | 会計年度開始時にリセット；デフォルトは4月（日本の慣例） |
...
このフォーマットでは:
- 変数は `%% <param> %%` 内に記述されます。
- `#:0>N` サフィックスは値を `N` 文字幅に左ゼロ埋めします（例: `#:0>3` は `5` を `005` にします）。サフィックスを省略すると値はそのまま使用されます。
...
- `rotateBy?: RotateByEnum`
  ...
  - 説明: シーケンスカウンターがリセットされるタイミングを決定します。詳細は上の `generateSequenceItem` のローテーション戦略表を参照してください。
