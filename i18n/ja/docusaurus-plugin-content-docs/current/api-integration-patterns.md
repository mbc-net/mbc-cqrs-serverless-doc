...
## SDK生成のセットアップ {#sdk-generation-setup}
...
export default defineConfig({
  ...
  input: './openapi.json', // またはOpenAPI仕様のURL
  output: {
...
## 生成されるSDK構造 {#generated-sdk-structure}
...
client.interceptors.request.use((request) => {
  const tenantCode = getTenantFromStore(); // Zustandストアから取得
  if (tenantCode) {
...
## React Query統合 {#react-query-integration}
...
## マルチテナントAPI呼び出し {#multi-tenant-api-calls}
...
## ファイルアップロード統合 {#file-upload-integration}
...
export const queryClient = new QueryClient({
  defaultOptions: {
    ...
    mutations: {
      retry: false, // デフォルトでミューテーションを再試行しない
    },
  },
});
```
