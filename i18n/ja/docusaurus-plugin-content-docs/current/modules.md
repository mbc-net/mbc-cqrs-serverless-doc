...
## モジュールコンポーネント {#module-components}
...
## フレームワークモジュール {#framework-modules}
...
| `DirectoryStorageModule` | `@mbc-cqrs-serverless/directory` | S3によるファイル・フォルダ管理 |
| `SurveyTemplateModule` | `@mbc-cqrs-serverless/survey-template` | 調査テンプレート管理 |

### サポートモジュール

| モジュール | パッケージ | 用途 |
|--------|---------|---------|
| `QueueModule` | `@mbc-cqrs-serverless/core` | SNSパブリッシュとSQS送受信メッセージング |
| `NotificationModule` | `@mbc-cqrs-serverless/core` | SESによるメール送信とAppSyncによるリアルタイムWebSocket通知 |
| `SettingModule` | `@mbc-cqrs-serverless/ui-setting` | ユーザーインターフェース設定の保存 |

## 動的モジュール登録 {#dynamic-registration}
...
SequencesModule.register({
  enableController: true, // Enable built-in sequence REST endpoints (組み込みシーケンスRESTエンドポイントを有効化)
})
```
...
TenantModule.register({
  enableController: true, // Enable built-in tenant REST endpoints (組み込みテナントRESTエンドポイントを有効化)
  dataSyncHandlers: [TenantRdsSyncHandler], // Optional: Data sync handlers for RDS synchronization (オプション: RDS同期用データ同期ハンドラー)
})
```
...
DirectoryStorageModuleは、権限管理・バージョン履歴・署名付きURL生成を備えたS3ベースのファイル・フォルダ管理機能を提供します。
...
| オプション | 型 | デフォルト | 説明 |
|--------|------|---------|-------------|
| `enableController` | `boolean` | `false` | 組み込みディレクトリRESTエンドポイントを有効化 |
| `prismaService` | `Type<PrismaService>` | enableController が true の場合は必須 | アプリケーションのPrismaServiceクラス |
| `dataSyncHandlers` | `Type[]` | `[]` | データ同期ハンドラークラス |

### SurveyTemplateModule

SurveyTemplateModuleは、テキスト・ラジオボタン・チェックボックス・評価など複数の質問タイプをサポートする調査テンプレート管理機能を提供します。
...
| オプション | 型 | デフォルト | 説明 |
|--------|------|---------|-------------|
| `enableController` | `boolean` | `false` | 組み込み調査テンプレートRESTエンドポイントを有効化 |
| `prismaService` | `Type<PrismaService>` | enableController が true の場合は必須 | アプリケーションのPrismaServiceクラス |
