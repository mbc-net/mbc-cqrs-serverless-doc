...
export interface Config {
  ...
  domain: {
    http: string;      // 例: api.example.com
    appsync: string;   // 例: graphql.example.com
  };

  userPoolId?: string;  // オプション: 既存のCognitoプールを使用
  ...
  rds: {
    accountSsmKey: string;  // DB認証情報のSSMパラメータ
    endpoint: string;
    dbName: string;
  };
  ...
  wafArn?: string;  // オプション: CloudFront用WAF
