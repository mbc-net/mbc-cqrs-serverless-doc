...
パーティション間のデータ分散を決定するDynamoDBのプライマリキーコンポーネント。MBC CQRS Serverlessでは、通常`tenantCode#TYPE`形式（例：`tenant001#ORDER`）。

### ソートキー（SK）

パーティション内での範囲クエリを可能にするDynamoDBのセカンダリキーコンポーネント。MBC CQRS Serverlessでは、データテーブルでは通常`TYPE#code`形式、コマンドテーブルでは`TYPE#code@N`形式。
