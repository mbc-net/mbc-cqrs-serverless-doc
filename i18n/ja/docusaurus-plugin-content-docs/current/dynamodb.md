...
## テーブルアーキテクチャ {#table-architecture}
...
| `session.json` | `session` | Read-Your-Writes セッションの追跡（v1.2.0以降）。[コマンドサービス](/docs/command-service#read-your-writes)を参照 |
...
```bash
# DynamoDB Local を起動（docker-compose に含まれています）
docker-compose up -d dynamodb-local

# DynamoDB Local 管理 UI にアクセス
