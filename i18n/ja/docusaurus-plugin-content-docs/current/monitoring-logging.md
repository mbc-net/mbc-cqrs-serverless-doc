...
```typescript
// The framework's RequestLogger is used automatically (フレームワークのRequestLoggerは自動的に使用されます)
// Log output includes: context, requestId, ip, tenantCode, userId (ログ出力には以下が含まれます: context、requestId、ip、tenantCode、userId)
...
export class YourService {
  ...
  async doSomething() {
    // These logs automatically include user context in Lambda (これらのログはLambdaでユーザーコンテキストを自動的に含みます)
...
## CloudWatchアラーム {#cloudwatch-alarms}
...
// SNS topic for notifications (通知用SNSトピック)
const alertTopic = new sns.Topic(this, 'AlertTopic');

// Lambda error alarm (Lambdaエラーアラーム)
...
// High latency alarm (高レイテンシーアラーム)
...
// DynamoDB throttling alarm (DynamoDBスロットリングアラーム)
...
// Enable X-Ray for API Gateway (API GatewayのX-Rayを有効化)
...
// Instrument AWS SDK v3 clients individually (AWS SDK v3クライアントをインストルメント)
const dynamoClient = AWSXRay.captureAWSv3Client(new DynamoDBClient({}));

// Instrument HTTP calls (HTTPコールをインストルメント)
AWSXRay.captureHTTPsGlobal(require('http'));
AWSXRay.captureHTTPsGlobal(require('https'));

// Add custom annotations (カスタムアノテーションを追加)
...
// Create a centralized log group (集中ロググループを作成)
const centralLogGroup = new logs.LogGroup(this, 'CentralLogs', {
  logGroupName: `/app/${props.appName}/central`,
  retention: logs.RetentionDays.THREE_MONTHS,
});

// Assume lambdaLogGroup and logProcessorFunction are already defined above (lambdaLogGroupとlogProcessorFunctionは上で定義済みと仮定)
