...
:::info バージョン情報
AP011〜AP015 は [v1.2.x](/docs/changelog#v120)（publishSync の null チェック、非推奨API、TaskModule 重複）で追加されました。AP016〜AP020 は [v1.2.5](/docs/changelog#v125) で追加。AP021（publishAsync 後のイベント発行）は [v1.2.6](/docs/changelog#v126) で追加。AP022〜AP025（eval、シェルインジェクション、HTTPタイムアウト、機密情報ロギング）とAP026（@NotificationTransport クラスへの @Injectable の二重指定）は [v1.3.0](/docs/changelog#v130) で追加。AP027（@GroupRoleResolver クラスへの @Injectable の二重指定）は [v1.3.1](/docs/changelog#v131) で追加されました。
:::
