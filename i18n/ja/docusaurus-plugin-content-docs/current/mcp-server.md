...
:::info バージョン情報
AP011〜AP015 は [v1.2.x](/docs/changelog#v120)（publishSync の null チェック、非推奨API、TaskModule 重複）で追加されました。AP016〜AP020 は [v1.2.5](/docs/changelog#v125) で追加。AP021（publishAsync 後のイベント発行）は [v1.2.6](/docs/changelog#v126) で追加。AP022〜AP025（eval、シェルインジェクション、HTTPタイムアウト、機密情報ロギング）とAP026（@NotificationTransport クラスへの @Injectable の二重指定）は [v1.3.0](/docs/changelog#v130) で追加。AP027（@GroupRoleResolver クラスへの @Injectable の二重指定）は [v1.3.1](/docs/changelog#v131) で追加。AP028（DataSyncHandler の重複登録）、AP029（`@DataSyncHandler` の予約済み `'dynamodb'` タイプ）、AP030（`@DataSyncHandler` の完全修飾テーブル名）は [v1.3.2](/docs/changelog#v132) で追加されました。
:::
...
| AP014 | 削除された `genNewSequence()` の呼び出し（v1.1.0で削除） | 高 | AP010 |
...
| AP023 | 文字列連結で構築されたシェルコマンド | 重大 | — |
| AP024 | タイムアウト未設定のHTTPリクエスト | 中 | — |
| AP025 | `process.env` やリクエストオブジェクト全体のロギング | 高 | — |
| AP026 | `@NotificationTransport` クラスへの `@Injectable` の二重指定 | 高 | — |
| AP027 | `@GroupRoleResolver` クラスへの `@Injectable` の二重指定 | 高 | — |
| AP028 | DataSyncHandler の重複登録 | 高 | — |
| AP029 | `@DataSyncHandler` の予約済み `'dynamodb'` タイプ | 高 | — |
| AP030 | `@DataSyncHandler` の完全修飾テーブル名 | 高 | — |
...
| `/mbc-review` | ベストプラクティスとアンチパターン（22パターン）に基づくコードレビュー |
...
これらは `/mbc-review` のレビューおよび人向けドキュメントで使用される **skill-doc AP番号** です。`mbc_check_anti_patterns` ツールが出力する **検出器AP番号** とは別の番号体系です。検出器→skill-doc のマッピングは上記の[アンチパターン検出表](#code-analysis-tools)を参照してください。AP021 は [v1.2.6](/docs/changelog#v126) で追加されました。AP022 は [v1.3.1](/docs/changelog#v131) で追加されました。
...
| AP022 | グループベースのロールリゾルバーの誤った実装（v1.3.1以降） | エラー |
...
**機能：**
- バージョン移行マトリックス（v1.0.16 から v1.3.1）
- 各バージョンの詳細な移行ガイド
- 非推奨APIの移行手順
