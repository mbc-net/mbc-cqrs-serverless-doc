...
@Module({
  imports: [
    TaskModule.register({
      ...
      enableController: true, // Optional: enable REST endpoints for task management (タスク管理REST APIエンドポイントを有効化 - 省略可)
    }),
  ],
...
| `QUEUED` | タスクは処理のためにキューに入れられました |
| `STARTED` | タスクの実行が開始されました |
| `PROCESSING` | タスクは現在処理中です |
| `FINISHED` | タスクの実行が終了しました |
