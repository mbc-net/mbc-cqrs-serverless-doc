...
## バージョン競合の原因 {#what-causes-conflicts}
...
// First, get the current item to know its version (まず、現在のアイテムを取得してバージョンを確認)
const currentItem = await this.dataService.getItem({ pk, sk });

const updateCommand: CommandPartialInputModel = {
  pk: currentItem.pk,
  sk: currentItem.sk,
  version: currentItem.version, // Include current version (現在のバージョンを含める)
  name: 'Updated Name',
};
...
const command: CommandInputModel = {
  ...
  type: 'CAT',
  name: 'Updated Name',
  version: VERSION_LATEST, // Auto-resolve to latest version (最新バージョンに自動解決)
  attributes,
};
...
const newCatCommand = new CatCommandDto({
  ...
  type: 'CAT',
  name: 'New Cat',
  version: VERSION_FIRST, // 0 - indicates new item (0 - 新規アイテムを示す)
  attributes,
});
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (
        ...
        // Wait before retry (exponential backoff) (リトライ前に待機（指数バックオフ）)
...
async function exponentialBackoff(attempt: number): Promise<void> {
  const baseDelay = 100; // Base delay in milliseconds (ミリ秒単位の基本遅延)
  const maxDelay = 5000; // Maximum delay (最大遅延)

  // Calculate delay with exponential backoff (指数バックオフで遅延を計算)
  const delay = Math.min(baseDelay * Math.pow(2, attempt), maxDelay);

  // Add random jitter to prevent thundering herd (サンダリングハードを防ぐためにランダムジッターを追加)
...
## VERSION_FIRSTとVERSION_LATESTの使い分け {#version-first-vs-latest}
...
const updateCommand = {
  pk: 'CAT#tenant1',
  sk: 'CAT#001',
  version: VERSION_LATEST, // -1, auto-resolves (-1、自動解決)
  // ... その他のフィールド
};
```
...
async function updateItem(item, changes) {
  try {
    const response = await fetch(`/api/items/${item.id}`, {
      ...
      body: JSON.stringify({
        ...changes,
        version: item.version, // Include version (バージョンを含める)
      }),
    });

    if (response.status === 409) {
      // Handle conflict - refresh and show user the changes (競合を処理 - 更新してユーザーに変更を表示)
    ...
    // Handle other errors (その他のエラーを処理)
    throw error;
  }
}
```
