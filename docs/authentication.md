...
{{**Impact on partition keys:** Since tenant codes are typically included in partition keys (e.g., `tenantCode#PRODUCT`), this normalization affects data access. If your existing data uses uppercase tenant codes in keys, queries will fail to find that data.}}
